			...
			struct ktermios *old)
{
	...
	if( up->pdev->id == 1 ){
		...
		for_TIOCSRS485_baud_rate[up->pdev->id]=baud_rate;
		// printk(KERN_NOTICE 
		//  "[%s](%d) debug in serial_omap_set_termios id[%d] baud_rate-1[%d]", __FILE__, __LINE__,
		//  	up->pdev->id, for_TIOCSRS485_baud_rate[up->pdev->id] );
	}
...
serial_omap_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
 struct uart_omap_port *up = (struct uart_omap_port *)port;
	...
	case TIOCSRS485: // #define TIOCSRS485	0x542F
		// printk(KERN_NOTICE 
