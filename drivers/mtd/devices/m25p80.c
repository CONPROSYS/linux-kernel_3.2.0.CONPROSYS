...
// #define M25P80_ORIGINAL ///< 2016.06.10 original source

/* Flash opcodes. */
#define	OPCODE_WREN		0x06	/* Write enable */
#define	OPCODE_RDSR		0x05	/* Read status register */
#define	OPCODE_RDFSR	0x70	///< 2016.06.22
...
#define FSR_RDY	0x80	///< 2016.06.22
...
{
//	printk(KERN_WARNING "[%s](%d)erase_chip in!!!",__FILE__,__LINE__); // 12.25
...
{
// printk(KERN_WARNING "[%s](%d)erase_sector in!!!",__FILE__,__LINE__); // 12.25
...
// printk(KERN_WARNING "[%s](%d)flash->command[0]=%x offset[%x]!!!",__FILE__,__LINE__, flash->command[0], offset); // 12.25
	m25p_addr2cmd(flash, offset, flash->command);

	spi_write(flash->spi, flash->command, m25p_cmdsz(flash));

// printk(KERN_WARNING "[%s](%d)erase_sector out!!!",__FILE__,__LINE__); // 12.25
	return 0;
}
...
// printk(KERN_WARNING "[%s](%d)m25p80_erase in!!!",__FILE__,__LINE__); // 12.25
	pr_debug("%s: %s at 0x%llx, len %lld\n", dev_name(&flash->spi->dev),
			__func__, (long long)instr->addr,
			(long long)instr->len);

// 2016.06.10
#ifdef M25P80_ORIGINAL
	/* sanity checks */
...
// printk(KERN_WARNING "[%s](%d)m25p80_erase out!!!",__FILE__,__LINE__); // 12.25
	return 0;
}
		...
		size_t *retlen, const u_char *buf)
{
	...
	size_t actual;
	int cmd_sz, ret;
// printk(KERN_WARNING "[%s](%d)sst_write in!!!",__FILE__,__LINE__); // 12.25
...
time_out:
	mutex_unlock(&flash->lock);
// printk(KERN_WARNING "[%s](%d)sst_write out ret(%d)!!!",__FILE__,__LINE__,ret); // 12.25
	return ret;
}
...
#define	M25P80_USE_FSR	0x80	// 2016.06.22 /* use flag status register */
};
...
printk(KERN_INFO "m25p_remove in!!!"); // 2015.01.14
