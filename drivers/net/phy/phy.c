...
static int led1=1;
	...
	printk(KERN_CONT "\n");
  // 2015.1.10
  if( memcmp( dev_name(&phydev->dev), "0:00", 4 )==0 ){
	// 2015.1.10
	if( led0 ){
	  ret = gpio_request(LAN_SPEED_LED0, "LAN_SPEED_LED0");
	  if (ret) {
		printk(KERN_ERR "%s: failed to request GPIO for LAN_SPEED_LED0 port "
		       "power control: %d\n", __func__, ret);
 		return;
	  }
	  led0=0;
	...
	if( phydev->speed == 100 )
		gpio_direction_output(LAN_SPEED_LED0, 1); // on
	else
...
// update 2015.02.28 LED ctrl update
  ...
  if( memcmp( dev_name(&phydev->dev), "0:01", 4 )==0 ){
	if( led1 ){
	  ...
	  if (ret) {
		printk(KERN_ERR "%s: failed to request GPIO for LAN_SPEED_LED1 port "
		       "power control: %d\n", __func__, ret);
 		return;
	  }
	  led1=0;
	...
	if( phydev->speed == 100 )
		gpio_direction_output(LAN_SPEED_LED1, 1); // on
	else
